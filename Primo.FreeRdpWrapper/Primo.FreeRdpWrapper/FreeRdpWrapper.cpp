...
namespace FreeRdpClient
{
	...
	BOOL OnBeginPaint(rdpContext* context)
	{
		rdpGdi* gdi;
		gdi = context->gdi;
	...
	BOOL OnEndPaint(rdpContext* context)
	{
		rdpGdi* gdi;
		DesktopImageData imageData;
		size_t imageByteCount;
		pOnImageUpdated callback = (pOnImageUpdated)context->paddingA[0];
		...
		imageByteCount = (size_t)gdi->stride * gdi->height;
	...
	BOOL OnPostConnect(freerdp* instance)
	{
		...
		instance->update->BeginPaint = OnBeginPaint;
		instance->update->EndPaint = OnEndPaint;

		return TRUE;
	}

	void OnPostDisconnect(freerdp* instance)
	{
		gdi_free(instance);
	}
	...
	void PrepareRdpContext(rdpContext* context, const ConnectOptions* rdpOptions)
	{
		...
		if (rdpOptions->OnImageUpdatedPtr)
			context->paddingA[0] = (UINT64)rdpOptions->OnImageUpdatedPtr;

		context->instance->PostConnect = OnPostConnect;
		context->instance->PostDisconnect = OnPostDisconnect;
	}
